...
**Current status of the installed package:** the package loads and registers the `/agentworks` command and model-callable `agentworks` tool (see `src/extension/index.ts`). Parent sessions use a private default runtime root under `~/.pi/agent/agentworks/runtime`; `AGENTWORKS_RUNTIME_ROOT` remains available for explicit overrides. Status requests use the authenticated controller read gateway and launch creates a durable planning run through the authenticated controller. In a Herdr/Pi session, HIGH launches pass the active parent model/provider into the first production orchestration tick; missing model/workspace context is reported as an error rather than an apparent launch. Child model transport across the isolated network namespace, completion, recovery, and the remaining management actions are still under live validation. See [Project status](#project-status--roadmap) below.
