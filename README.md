...
- **The parent Pi extension's real behavior** — status can use the authenticated controller read gateway and launch can create a durable planning run when `AGENTWORKS_RUNTIME_ROOT` is configured, while agent orchestration/mutating actions, the persistent right-side overlay, narrow-terminal fallback, and run restoration on Pi restart remain unimplemented (P8).
