...
async function bytesToHex(bytes: Buffer | Uint8Array): Promise<string> {
  return new Promise((resolveBytesToHex) => {
    ...
    const hexArray = Array(array.length)
      .fill(0)
      .map((_, i) => array[i].toString(16).padStart(2, "0"));
...
export function connectRemoteAgentBridge(
  options: RemoteAgentBridgeOptions,
): RemoteAgentBridge {
  ...
  log(
    `connected ${hexToken}... ${hexRunId}... ${hexAgent}... ${hexSession} at ${hexSocketPath.slice(0, 32)}...`,
  );
  ...
  function onConnect(): void {
    ...
    const payload = {
      payload: { runId, agentId, sessionId, version: 1 },
      id: requestId,
      action: "agent.hello",
    };
    try {
